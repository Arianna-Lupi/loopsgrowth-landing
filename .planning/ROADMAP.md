...
Plans:
- [ ] 01-01-PLAN.md: Esqueleto caminable (Astro 7 y Tailwind 4, copy en `landing.es.yaml` con esquema, formulario real de ClickUp en `#agenda`, entorno con `noindex` y Outfit tras `--font-brand`)
- [ ] 01-02-PLAN.md: Guardas de build (`tokens.css` con `check-contrast.mjs` y `check-copy.mjs` en `prebuild`)
- [ ] 01-03-PLAN.md: Superficies UI y accesibilidad base (CTA con foco al h2, skip links, header, enlace de respaldo con `noscript`, verificación con Playwright)
...
|-------|----------------|--------|-----------|
| 1. Fundaciones y formulario funcionando | 0/4 | Not started | - |
