...
| Content layer | `src/content/landing.es.yaml` + colección de contenido con esquema Zod (`{text, status}`, `status: verified \| pending`) | Un error de copy rompe el build en vez de publicarse. Ari edita un solo archivo |
...
| Fonts | Outfit vía `fontProviders.fontsource()` tras `--font-brand` (pesos 400 y 700, subsets latin y latin-ext) | Respaldo hasta tener licencia web de Hurme; el cambio a `fontProviders.local()` no toca componentes |
...
| Build guards | `prebuild` corre `scripts/check-contrast.mjs` y `scripts/check-copy.mjs` (esta última solo bloquea con `PUBLIC_ENV=production`) | Protegen contraste y copy desde el día uno, antes de que existan las secciones |
...
| Directory layout | `src/{layouts,components,content,lib,scripts,styles,pages}`, `scripts/` (Node de build/QA), `tests/{guards,e2e}`, `docs/a11y/` (Fase 3) | Una sección, un archivo; ningún texto fuera del YAML |
